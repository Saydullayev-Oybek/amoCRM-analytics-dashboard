...
- **Config in containers:** `AMOCRM_CONFIG_DIR` env points to the mounted config
  dir; `config.py` reads it (defaults to `.` for local CLI runs). `PYTHONPATH`
  exposes the `amocrm` package; `DLT_DATA_DIR` holds dlt state/logs.
- **Docker gotcha:** the compose `postgres` is Airflow's *own* metadata DB —
  separate from the analytics DB in `postgres_config.json`. If the analytics DB
  runs on the host, use `host.docker.internal` as its host (not `localhost`).
